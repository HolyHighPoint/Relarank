...
static void fireAnimation(QPropertyAnimation & animation, qreal targetValue,
                          qreal startValue, qreal duration,
                          const QEasingCurve & curve);
static
relarank::Plug *
findClosestPlug(const QPointF & pos, const QSet < relarank::Plug * >&plugs,
                relarank::Plug * closest);
...
Node::Node(Scene * scene, const QString & displayName,
           const QUuid & uuid): QGraphicsObject(nullptr),
    m_scene(scene), m_displayName(displayName),
    m_uniqueId(uuid.isNull() ? QUuid::createUuid() : uuid),
    m_outgoingExpansionFactor(0.), m_incomingExpansionFactor(0.),
    m_perimeter(nullptr), m_allPlugs(QMap < QString, Plug * >()),
    m_outgoingPlugs(QSet < Plug * >()), m_incomingPlugs(QSet < Plug * >()),
    m_straightEdges(QSet < StraightEdge * >()), m_label(nullptr),
    ...
    m_lastExpansionState(NodeExpansion::NONE)
{
    ...
    connect(&m_outgoingCollapseAnimation, SIGNAL(finished()), this,
            SLOT(hasCompletelyCollapsed()));
    connect(&m_incomingCollapseAnimation, SIGNAL(finished()), this,
            SLOT(hasCompletelyCollapsed()));
}
...
void Node::updateStyle()
{
    ...
    m_perimeter->
    setOpacity(qMax(m_outgoingExpansionFactor, m_incomingExpansionFactor)
               * Perimeter::getMaxOpacity());
...
void Node::mouseMoveEvent(QGraphicsSceneMouseEvent * event)
{
    ...
    for (QGraphicsItem * selectedItem : scene()->selectedItems()) {
        Node *selectedNode =
            qobject_cast < Node * >(selectedItem->toGraphicsObject());
        if (selectedNode) {
...
QString Node::getUniquePlugName(const QString & name, const Plug * plug)
{
    ...
    do {
        ...
        if (testPlug == plug) {
            // if the user wants to rename x_2 to x while x already exists, that's a no-op
            return uniqueName;
        }
    } while (testPlug);
    return uniqueName;
...
void Node::setExpansion(NodeExpansion newState)
{
    ...
    if (newState == NodeExpansion::NONE) {
        ...
        fireAnimation(m_incomingCollapseAnimation, 0.0,
                      m_incomingExpansionFactor, s_collapseDuration,
                      s_collapseCurve);
        fireAnimation(m_outgoingCollapseAnimation, 0.0,
                      m_outgoingExpansionFactor, s_collapseDuration,
                      s_collapseCurve);
    } else {
        ...
        if (newState == NodeExpansion::IN) {
            ...
            fireAnimation(m_incomingExpandAnimation, 1.0,
                          m_incomingExpansionFactor, s_expandDuration,
                          s_expandCurve);
            fireAnimation(m_outgoingCollapseAnimation, 0.0,
                          m_outgoingExpansionFactor, s_collapseDuration,
                          s_collapseCurve);
            ...
            fireAnimation(m_incomingCollapseAnimation, 0.0,
                          m_incomingExpansionFactor, s_collapseDuration,
                          s_collapseCurve);
            fireAnimation(m_outgoingExpandAnimation, 1.0,
                          m_outgoingExpansionFactor, s_expandDuration,
                          s_expandCurve);
        } else {
            // BOTH
            m_outgoingCollapseAnimation.stop();
...
void Node::arrangePlugs()
{
    ...
    QVector < qreal > zoneDirections(evenZoneCount);
    {
        ...
        qreal zoneSpan =
            (M_PI - (2. * halfDeadAngle) -
             ((halfZoneCount + 1) * gapAngle)) / halfZoneCount;
        // upper half
    ...
    for (Plug * plug : plugs) {
        ...
        plugDirections.append(QPair < Plug *,
                              qreal > (plug,
                                       qAtan2(-plugTarget.y(),
                                              plugTarget.x())));
    }
    ...
    if (connectedPlugCount > 0) {
        ...
        for (int row = 0; row < connectedPlugCount; ++row) {
            ...
            for (int column = 0; column < evenZoneCount; ++column) {
                qreal cost =
                    angularDistance(plugDirection,
                                    zoneDirections[column]) *
                    plug->getArrangementPriority();
                costTable[(row * evenZoneCount) + column] = cost * cost;
            }
        }
        // modify the trivial path to the semi-optimal one
        int i = 0;
        for (int zoneIndex : relarank::arrangePlugs(costTable, connectedPlugCount,
                evenZoneCount)) {
    ...
    if (plugCount < evenZoneCount) {
        ...
        qreal newZoneSpan =
            (M_PI - (2. * halfDeadAngle) -
             ((halfZoneCount) * gapAngle)) / (halfZoneCount - 1);
        qreal currentAngle =
            offset + halfDeadAngle + gapAngle + (newZoneSpan / 2);
    ...
    for (int plugIndex = 0; plugIndex < plugCount; ++plugIndex) {
        ...
        plugs.at(plugIndex)->
        defineShape(QVector2D(qCos(angle), -qSin(angle)),
                    plugSweepAngle);
    }
}
...
{
    return m_label->getHeight() / (m_perimeter->getRadius() -
                                   (Plug::getWidth() * 1.5));
}
...
void Node::adjustRadius()
{
    ...
    qreal requiredArcLength
        = (s_plugSweep * plugCount)
          + (s_plugGap * (plugCount + 2)) + (deadArcLength * 2);
...
static void
fireAnimation(QPropertyAnimation & animation, qreal targetValue,
              qreal startValue, qreal duration, const QEasingCurve & curve)
{
    ...
    animation.
    setDuration(qMax(0., qAbs(targetValue - startValue) * duration));
    animation.start();
}
                ...
                relarank::Plug * closest)
{
    ...
    if (closest) {
        minDistSq =
            QVector2D((closest->getNormal()).toPointF() -
                      pos).lengthSquared();
    } else {
        minDistSq = DBL_MAX;
    }
    // find closest plug to pos
    for (relarank::Plug * plug : plugs) {
        // skip incoming plugs that already have an edge connected
        if ((plug->getDirection() == relarank::PlugDirection::IN)
            && (plug->getEdgeCount() > 0)) {
            continue;
        }
        qreal distSq =
            QVector2D((plug->getNormal()).toPointF() - pos).lengthSquared();
        if (distSq < minDistSq) {
            minDistSq = distSq;
            closest = plug;
        }
...
static qreal
angularDistance(qreal alpha, qreal beta)
{
