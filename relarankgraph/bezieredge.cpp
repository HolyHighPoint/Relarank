...
void BezierEdge::placeArrowAt(qreal fraction)
{
    ...
    QPointF edgeDirection =
        m_path.pointAtPercent(qMin(pos + VERY_SMALL_DELTA, 1.)) - edgeCenter;
    m_arrow->setTransformation(edgeCenter,
                               atan2(edgeDirection.y(),
                                     edgeDirection.x()));
}
...
QPointF BezierEdge::getCtrlPointFor(Plug * plug)
{
    ...
    case PlugDirection::BOTH:
        factor =
            qMax(plug->getNode()->getIncomingExpansionFactor(),
                 plug->getNode()->getOutgoingExpansionFactor());
        break;
    default:
        factor = 0.0;		//
    ...
    qreal ctrlDistance =
        qMin(s_maxCtrlDistance, manhattanLength * s_ctrlExpansionFactor);
    return plug->scenePos() +
           ((plug->getNormal() * ctrlDistance).toPointF() * factor);
}
