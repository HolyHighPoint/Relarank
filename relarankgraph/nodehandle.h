...
/// \file nodehandle.h
///
/// \brief Contains the definition of the relarank::NodeHandle class.
///
...
///
/// \brief A handle object for a relarank::Node.
///
/// Is a thin wrapper around a pointer but with a much nicer, outward facing interface than the real Node.
///
/// See \ref relarank::SceneHandle "SceneHandle" for more details on handles.
///
...
signals:
    ...
    ///
    /// \brief Emitted, when the \ref relarank::View::s_activationKey "activation key" is pressed while the managed Node
    /// is selected.
    ///
    void nodeActivated();
    ...
    ///
    /// \brief Emitted, when the \ref relarank::View::s_removalButton "removal button" is pressed on the managed Node.
    ///
...
///
inline uint qHash(const relarank::NodeHandle& key)
{
