...
static relarank::Plug* findClosestPlug(const QPointF& pos, const QSet<relarank::Plug*> &plugs, relarank::Plug* closest);
...
void Node::arrangePlugs()
{
    ...
    if(connectedPlugCount>0){
        ...
        int i = 0;
        for(int zoneIndex : relarank::arrangePlugs(costTable, connectedPlugCount, evenZoneCount)){
...
static relarank::Plug* findClosestPlug(const QPointF& pos, const QSet<relarank::Plug*> &plugs, relarank::Plug* closest)
{
    ...
    for(relarank::Plug* plug : plugs){
        ...
        if((plug->getDirection()==relarank::PlugDirection::IN)&&(plug->getEdgeCount()>0)){
            continue;
        }
