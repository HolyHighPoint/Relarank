                      ...
                      std::vector<graph_edge> &edge, std::vector < std::pair<int, int> > &location)
{
    ...
    for(size_t i=0;i<edge.size();i++){
        ...
        if(!boyer_myrvold_planarity_test(boyer_myrvold_params::graph = g, boyer_myrvold_params::embedding = &embedding_storage[0]))g=gt;
    }
    embedding_storage = embedding_storage_t(num_vertices(g));
    ...
    for(tie(vi,vi_end) = vertices(g); vi != vi_end; vi++){
        ...
        location.push_back(make_pair(coord.x*80, coord.y*80));
